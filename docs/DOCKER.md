...
| 内容 | 容器路径 | 宿主机位置 | 生命周期 |
| --- | --- | --- | --- |
| 数据库、Session、API 凭据 | `/data` | Docker 命名卷 | `docker compose down` 后保留 |
...
- API ID/API Hash 不做应用层加密，以 `0600` 文件保存在 `/data/credentials.bin`；能够读取 Docker 数据卷的主体可以读取这些凭据。
- 请限制 Docker 守护进程、宿主机管理员和卷备份的访问权限，不要发布、复制或共享 `/data` 卷。
...
重点检查 `/data` 和 `/exports` 权限，以及端口 `4173` 是否已被占用。
