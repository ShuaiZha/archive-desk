...
容器模式由 `ARCHIVEDESK_CONTAINER=1` 启用，前端静态文件由 `ARCHIVEDESK_STATIC_DIR` 同源提供。默认 Compose 配置把状态保存在 `/data`，把导出结果写入 `/exports`。容器内 API 凭据不做应用层加密，以 `0600` 文件保存；必须保护 Docker 主机和数据卷访问权限。
